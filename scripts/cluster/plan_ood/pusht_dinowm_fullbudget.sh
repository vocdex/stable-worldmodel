#!/bin/bash
#SBATCH -J swm_ood_pusht_dwm_fb
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --nodes=1
#SBATCH --partition=a100-galvani
#SBATCH --gres=gpu:a100:1
#SBATCH --time=0-02:00          # n_steps=30: measured ~88 min/cell (CEM ~33 min + ~55 min env stepping)
#SBATCH --mem=64G
#SBATCH --array=0-74%12         # 25 cells x 3 seeds; %12 caps concurrent tasks at 12 (cluster limit)
#SBATCH --output=/mnt/lustre/work/martius/mot956/stable-worldmodel/logs/swm_ood_pusht_dwm_fb_%A_%a.out
#SBATCH --error=/mnt/lustre/work/martius/mot956/stable-worldmodel/logs/swm_ood_pusht_dwm_fb_%A_%a.err

# -----------------------------------------------------------------------------
# LEGACY DINO-WM (+proprio ckpt) PushT OOD matrix, NO PROPRIO COST (alpha=0) —
# FULL CEM BUDGET rerun of the robust_a0.0_n300_eps50_s{0,1,2} matrices.
#
# The original robustness sweep ran at N_STEPS=10 (a937320) while every other
# matrix (cube/pusht PreJEPA, legacy ood_matrix, dyn_cells) used 30 CEM
# iterations — so those results are not budget-matched. This rerun restores
# n_steps=30 and writes to a results dir that ENCODES the iteration count
# (robust_a0.0_n300i30_eps50_s*) so the two budgets can never be conflated.
#
...
#
# Prereqs on cluster (already in place from the original runs):
#   1. /mnt/lustre/work/martius/mot956/dino_wm/ (models/ + checkpoints/outputs/pusht)
#   2. datasets/pusht_expert_train.h5
#
# Submit:
#   sbatch scripts/cluster/plan_ood/pusht_dinowm_fullbudget.sh
# Override e.g.:
#   ALPHA=1.0 sbatch scripts/cluster/plan_ood/pusht_dinowm_fullbudget.sh   # with proprio cost
# -----------------------------------------------------------------------------
...
read -r -a SEEDS <<< "${SEEDS:-0 1 2}"   # matches the seeds of the n_steps=10 originals
...
N_STEPS=${N_STEPS:-30}           # FULL budget — matches all other 30-iteration matrices
TOPK=${TOPK:-30}
ALPHA=${ALPHA:-0.0}              # no proprio cost (visual-only planning cost)

# i${N_STEPS} in the dir name keeps this run distinct from the legacy
# 10-iteration robust_a0.0_n300_eps50_s* results (which omit the count).
RESULTS_DIR=$WORK_BIND_DIR/checkpoints/dino_wm_legacy_pusht/robust_a${ALPHA}_n${NUM_SAMPLES}i${N_STEPS}_eps${NUM_EVAL}_s${SEED}
