...
#SBATCH --time=0-04:00          # per-task budget; measured ~3h16m/cell (2x 5660s CEM + overhead) at num_eval=50, bs=4 — ~45 min leeway
#SBATCH --mem=64G
...
#
# Submit:
#   sbatch scripts/cluster/plan_ood/cube.sh
#
# Submit a subset (e.g. just cells 5,8,12 after a partial failure):
#   sbatch --array=5,8,12 scripts/cluster/plan_ood/cube.sh
#
# Override knobs via env:
#   EPOCH=20 NUM_EVAL=50 NUM_SAMPLES=300 sbatch scripts/cluster/plan_ood/cube.sh
# -----------------------------------------------------------------------------
