...
#SBATCH --output=/mnt/lustre/work/martius/mot956/stable-worldmodel/logs/swm_cube_dv3_epsweep_%A_%a.out
#SBATCH --error=/mnt/lustre/work/martius/mot956/stable-worldmodel/logs/swm_cube_dv3_epsweep_%A_%a.err

# -----------------------------------------------------------------------------
# DINOv3 baseline-only (in-distribution) epoch sweep: e5/e10/e15/e20 x seeds
# {0,1,2}. The OOD matrices default to EPOCH=10 because that won DINOv2's
# baseline sweep — DINOv3 inherited that choice untested. This sweep gives
# DINOv3 the same per-backbone epoch selection before concluding on the
# in-distribution gap (74 vs 64 at e10, seeds {0,1,2}).
#
# Checkpoints weights_epoch_{5,10,15,20}.pt already exist on the cluster from
# the training run; this is planning cost only (~3h/task).
#
# Task layout: EPOCH = task % 4, SEED = task / 4.
#
# Submit:
#   sbatch scripts/cluster/plan_baseline/cube_dinov3_epoch_sweep.sh
# Single seed only:
#   sbatch --array=0-3 scripts/cluster/plan_baseline/cube_dinov3_epoch_sweep.sh
# -----------------------------------------------------------------------------
...
RESULTS_DIR=$WORK_BIND_DIR/checkpoints/cube_dinov3_small_actiononly_cachedfeats_psmall/baseline_epoch_sweep
